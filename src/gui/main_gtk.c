...
match_colours pango_colours = {
    .end = "</span>",
    .changeover = "</span>",
    .exact = "<span color=\"blue\">",
    .guess = "<span color=\"olive\">",
    .unknown = "<span color=\"red\">",
...
void search_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (file_path && search_text && strlen(search_text) >= 2) {
        ...
        FILE *input_copy = fopen(file_path, "rb");
        if (input && input_copy) {
            ...
            search_matches_in_file(input, search_text, false,
                                   search_result_callback, &find_data);
            fclose(input);
            fclose(input_copy);
...
void app_main(GtkApplication *app, gpointer user_data) {
    ...
    gtk_grid_attach(GTK_GRID(search_settings_grid), rom_path_browse_button, 2,
                    0, 1, 1);
    ...
    gtk_grid_attach(GTK_GRID(search_settings_grid), search_string_label, 0, 1,
                    1, 1);
    ...
    g_object_set_property(G_OBJECT(search_string_entry), "activates-default",
                          &value);
    gtk_grid_attach(GTK_GRID(search_settings_grid), search_string_entry, 1, 1,
                    2, 1);
    ...
    GListStore *search_results_model =
        g_list_store_new(ASSS_TYPE_SEARCH_RESULT);
    ...
    // Search results box
    GtkWidget *search_results = gtk_column_view_new(
        GTK_SELECTION_MODEL(search_results_selection_model));
    ...
    GtkListItemFactory *result_column_factory =
        asss_make_result_column_factory();
    ...
    GtkWidget *save_button =
        gtk_button_new_with_mnemonic("S_ave (not implemented yet)");
