...
static void text_column_setup(GtkSignalListItemFactory *factory,
...
static void text_column_unbind(GtkSignalListItemFactory *factory,
                               GtkListItem *item, gpointer user_data) {
    (void)factory;
    (void)user_data;
...
static void text_column_teardown(GtkSignalListItemFactory *factory,
                                 GtkListItem *item, gpointer user_data) {
    (void)factory;
    (void)user_data;
...
static void address_column_bind(GtkSignalListItemFactory *factory,
                                GtkListItem *item, gpointer user_data) {
    (void)factory;
    (void)user_data;
    GtkWidget *label = gtk_list_item_get_child(item);
    ...
    gchararray text =
        g_strdup_printf("%08llx", (unsigned long long)result->address);
    gtk_label_set_text(GTK_LABEL(label), text);
    g_free(text);
}

static void result_column_bind(GtkSignalListItemFactory *factory,
                               GtkListItem *item, gpointer user_data) {
    (void)factory;
    (void)user_data;
    GtkWidget *label = gtk_list_item_get_child(item);
    AsssSearchResult *result = (AsssSearchResult *)gtk_list_item_get_item(item);
    gtk_label_set_markup(GTK_LABEL(label), result->text);
}
...
GtkListItemFactory *asss_make_address_column_factory(void) {
    ...
    g_signal_connect(factory, "teardown", G_CALLBACK(text_column_teardown),
                     NULL);
    return factory;
}
